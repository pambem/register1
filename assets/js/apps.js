...
function addSpinner(div, msg){
    
    var target = $(div).closest('.panel');
    if (!$(target).hasClass('panel-loading')) {
        var targetBody = $(target).find('.panel-body');
        var spinnerHtml = '<div class="panel-loader"><span class="spinner-small"></span></div>';
        $(target).addClass('panel-loading');
    ...
    $(div).append('<span style="float: right;" id="'+uniqid+'"><img src="imgs/spinner_w.gif">&nbsp;' + msg + '</span>');
...
function removeSpinner(spinnerId){
    ...
    var div = $(document.getElementById(spinnerId));
    var target = $(div).closest('.panel');
    $(target).removeClass('panel-loading');
    $(target).find('.panel-loader').remove();
    div.remove();
